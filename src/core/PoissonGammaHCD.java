...
public class PoissonGammaHCD {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		if(stage == 1)
		{
			...
			initializeForrest();
			//System.exit(0);
			populateInitialHeap();// NOTE: all the pairs of tree merges so far are just in the heap, not added to forrest, coz not all will be added to the forrest
			//System.exit(0);
	...
	{
		//FileInputStream fis = new FileInputStream("data/queriesLISTFromAOL_Gold_demo");
		FileInputStream fis = new FileInputStream(queryListFile);
	...
	public static void populateInitialHeap()
	{
		...
		for(int i=0;i<forrest.size();i++)
		{
			...
			for(int j=i+1;j<forrest.size();j++)
			{
				...
				if(pruning == 1) if(m.likelihood == -1) continue;
				heap.add(m);
		...
		while(heap.size() > 0)
		{
			...
			if(forrestMap.containsKey(new Integer(X.treeID)) && forrestMap.containsKey(new Integer(Y.treeID)))
			{
				...
				while(itr.hasNext())
				{
					...
					//if(M.likelihood > temp) System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\t"+M.likelihood+"__"+M.nodeList.size()+"____"+temp+"__"+I.nodeList.size());
					//if(pruning == 1) if(M.likelihood == -1) {System.out.println("----------------------SKIPPED -1");continue;}
					heap.add(M);
					if(M.nodeList.size() > maxTreeSize) {maxTreeSize = M.nodeList.size();finalTree = M;}
					//if(M.likelihood > 10) System.exit(0);
					...
					if(M.nodeList.size() == sizeNetwork || M.nodeList.size() > 800)// was ==866
					{
						finalTree = M;
						System.out.println("Final tree -- "+M.childTrees.size()+"_"+M.nChildren+"_"+M.nodeList.size());
					}
				}
	...
	public static Tree mergeTrees(Tree I, Tree J)
	{
		...
		if(pruning == 1) if(num1 == 0 && num2 == 0 && num3 == 0) {M.likelihood = -1; return M;}
		...
		if(num1 >= num2 &&  num1 >= num3)
		{
			...
			M.bayesFactorScore = getBayesFactorScoreForTree(M);
			//System.out.println("11111111\t"+M.nChildren+"\t"+M.nodeList.size());
		}
		else if(num2 >= num1 &&  num2 >= num3)
		{
			...
			M.likelihood = likelihood;
			M.bayesFactorScore = getBayesFactorScoreForTree(M);
			//System.out.println("2222222\t"+M.nChildren+"\t"+M.nodeList.size());
		}
		else// if (num3 >= num1 &&  num3 >= num2)
		{
			...
			M.likelihood = likelihood;
			M.bayesFactorScore = getBayesFactorScoreForTree(M);
			//System.out.println("333333333\t"+M.nChildren+"\t"+M.nodeList.size());
		}
		if(pruning==1) if(flag==1) M.likelihood = -1;
		return M;
	}
	...
	public static double get_fDmValue(Tree t)
	{
		...
		for(int i=0;i<size;i++)
		{
			...
			for(int j=i+1;j<size;j++)
			{
				...
				//count++;
				//if(temp>0) result*=temp; // changed from multiply to plus
			}
		}
	...
	public static double getTreeLikelihood(Tree t)
	{
		...
		else // if its not a leaf, then its a merge tree,
		{
			if(t.isInitialMerge)
			{
				...
				//System.out.println("r1/2/3="+r1+"_"+r2+"_"+r3);
				// now we have the r values, we 
				double affinity1=1, affinity2=1, affinity3=1;//not sure if they should be initialized to 1
				//if(r1!=0)
					affinity1 = getGammaPosterior(alpha1, beta1, r1);//what if r=0; shud they be initialized to 1?
				//if(r2!=0)
					affinity2 = getGammaPosterior(alpha2, beta2, r2);
				//if(r3!=0)
					affinity3 = getGammaPosterior(alpha3, beta3, r3);
				// now we have the affinities from each of the 3 relations, we need to calculate the likelihood of this tree (= with just two nodes)
				...
				if(likelihood > 1000000) {System.out.println("--~~~~~"+likelihood+" r1/2/3="+r1+"_"+r2+"_"+r3+" affinity 1/2/3:"+affinity1+"_"+affinity2+"_"+affinity3);System.exit(0);}
				if(pruning == 1) if((r1+r2+r3) == 0) likelihood=-1;
				return likelihood;//we donot assign this likelihood to the tree here coz the function calling it would be doing it anyway
			}
			else
			{
	...
	{
		FileInputStream fis = new FileInputStream(treeFile);// was _DEMO
        ObjectInputStream ois = new ObjectInputStream(fis);
	...
	{
		FileOutputStream fos = new FileOutputStream(treeFile);// was _DEMO
